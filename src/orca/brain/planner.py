...
def make_planner_node(menu: str, doc_list: str):
    ...
    def planner_node(notebook: Notebook) -> dict:
        ...
        prompt = (
            ...
            "several files hold sheets with the same names. Repeat the name "
            "EXACTLY AS THE USER SAID IT (\"Acme\", \"the survey\") — NEVER "
            "expand it into a document's full/official title from the catalog: "
            "a long title is made of the document's own commonest words and "
            "drowns the search engine's keyword matching.\n"
